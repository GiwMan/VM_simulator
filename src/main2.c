...
void insertPage(HPT *hpt1, HPT *hpt2, Queue *q,unsigned int page, int pid, char mode, int count, int *totalFrames) {
...
void LRUsimulation(char *algorithm, int maxFrames, int q, int maxTraces) {
    ...
    **/
    ...
    while(count < maxTraces) {
        ...
        insertPage(&h1, &h2, &q_lru, page, pid, status, count, &totalFrames);
    ...
    printf("\n    Total Page Faults:  %d\n    Total R:  %d\n    Total W:  %d\n",
        h1.pageFaults + h2.pageFaults, 
        h1.nReads + h2.nReads, 
        h1.nWrites + h2.nWrites
        );
        ...
        if(pid == 1) {
            ...
            // for pid = 2
             if(pageExists(h2, page) == 1) {
                ...
                // update hash table
                updateHashTablePage(&h2, page, status, 0);
            } else {
                h2.pageFaults++;
                totalFrames++;
